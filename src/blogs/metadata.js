import grcComplianceMgmt from "../assets/blogs/grc-compliance-management.jpg";
import masteringGrcRuleset from "../assets/blogs/mastering-grc-ruleset.jpg";
import regainedSapSecurity from "../assets/blogs/regained-sap-security-expert.jpg";
import integratingOkta from "../assets/blogs/integrating-okta.jpg";
import publicCloudAuthUpgrade from "../assets/blogs/public-cloud-auth-upgrade.jpg";
import auditControlsFail from "../assets/blogs/audit-controls-fail.jpg";
import configurationWithoutSpro from "../assets/blogs/configuration-without-spro.jpg";
import cpcVsSpro from "../assets/blogs/cpc-vs-spro.jpg";
import publicVsPrivateCloud from "../assets/blogs/public-vs-private-cloud.jpg";
import cybersecurityPodcast from "../assets/blogs/cybersecurity-insights.jpg";
import magicianMachine from "../assets/blogs/magician-and-machine.jpg";
import licenseOptimization from "../assets/blogs/licensing-optimization.jpg";
import starUsmmLawSlaw from "../assets/blogs/what-actually-optimizes.jpg";

const _placeholder = "https://placehold.co/600x400?text=SAP+Expert";

export const blogMetadata = [
  {
    id: "GrcComplianceManagement",
    title: "GRC Compliance Management in SAP: Powering Enterprise-Wide Governance, Risk, and Compliance",
    slug: "grc-compliance-management-in-sap-powering-enterprise-wide-governance-risk-and-compliance",
    metaDescription: "Powering Enterprise-Wide Governance, Risk, and Compliance.",
    excerpt: "GRC Compliance Management has become a strategic requirement for enterprises operating complex SAP landscapes.",
    image: grcComplianceMgmt,
    date: "2026-01-21",
    category: "sap-grc",
    author: "Raghu Boddu",
    views: "34",
    tags: ["grc for sap", "grc management", "GRC SAP module", "SAP critical access", "SAP governance", "SAP governance risk and compliance", "SAP GRC", "sap grc module", "sox compliance in sap"]
  },
  {
    id: "MasteringGrcRuleset",
    title: "Mastering SAP GRC Ruleset Manager: A Complete Overview",
    slug: "mastering-sap-grc-ruleset-manager-a-complete-overview",
    metaDescription: "A complete overview of the Ruleset Manager in SAP GRC Access Control 12.0 SP25.",
    excerpt: "Managing access risk effectively is one of the toughest challenges in Governance, Risk, and Compliance (GRC) frameworks.",
    image: masteringGrcRuleset,
    videoID: "l3MEn1n4rww",
    date: "2026-01-17",
    category: "sap-grc",
    author: "Raghu Boddu",
    views: "70",
    tags: ["Access Risk Analysis", "Audit and Compliance", "Ruleset Manager", "SAP GRC", "SAP Authorization", "SoD Analysis"]
  },
  {
    id: "RegainedSapSecurityExpert",
    title: "Regained SAP Security Expert!",
    slug: "regained-sap-security-expert",
    metaDescription: "SAP Security Expert is back home.",
    excerpt: "After a long and unexpected hiatus, SAP Security Expert is finally back home.",
    image: regainedSapSecurity,
    date: "2026-01-05",
    category: "sap-grc",
    author: "Raghu Boddu",
    views: "161",
    tags: ["SAP GRC", "SAP Security"]
  },
  {
    id: "IntegratingOkta",
    title: "Integrating Okta with SAP IAS/IPS by Raghu Boddu — Step-by-Step IAM Best Practices",
    slug: "integrating-okta-with-sap-ias-ips-by-raghu-boddu-step-by-step-iam-best-practices",
    metaDescription: "Step-by-Step IAM Best Practices for integrating Okta with SAP IAS/IPS.",
    excerpt: "The complexities of modern enterprise identity and access management (IAM) demand scalable, secure integrations.",
    image: integratingOkta,
    videoID: "EUikjPArefI",
    date: "2026-01-17",
    category: "sap-btp-security",
    subCategory: "sap-iag",
    author: "Raghu Boddu",
    views: "116",
    tags: ["IAM Solutions", "SAP BTP", "SAP CIS", "SAP IAG", "SAP IAM", "SAP IPS"]
  },
  {
    id: "PublicCloudAuthUpgrade",
    title: "SAP Public Cloud Authorisation Upgrade- Comprehensive IAM Release Strategy & Execution Guide",
    slug: "sap-public-cloud-authorisation-upgrade-comprehensive-iam-release-strategy-execution-guide",
    metaDescription: "Comprehensive IAM Release Strategy & Execution Guide for SAP Public Cloud.",
    excerpt: "Upgrading custom business roles represents the most complex and critical phase of the IAM release management process.",
    image: publicCloudAuthUpgrade,
    date: "2026-01-17",
    category: "sap-public-cloud",
    author: "Inderdeep Singh",
    views: "86",
    tags: ["Authorization upgrade in public cloud", "Best Practices", "SAP Public Cloud", "Upgrading SAP Public Cloud"]
  },
  {
    id: "AuditControlsFail",
    title: "Why Traditional SAP Audit Controls Fail in Public Cloud",
    slug: "why-traditional-sap-audit-controls-fail-in-public-cloud",
    metaDescription: "Why Traditional SAP Audit Controls Fail in Public Cloud.",
    excerpt: "Traditional SAP audits were designed for a world where customers owned the system end to end.",
    image: auditControlsFail,
    date: "2026-01-17",
    category: "sap-public-cloud",
    author: "Raghu Boddu",
    views: "60",
    tags: ["SAP Public Cloud", "Security Audit", "Cloud Controls"]
  },
  {
    id: "ConfigurationWithoutSpro",
    title: "Configuration Without SPRO: The New Audit Reality of SAP Public Cloud",
    slug: "configuration-without-spro-the-new-audit-reality-of-sap-public-cloud",
    metaDescription: "The New Audit Reality of SAP Public Cloud: Configuration Without SPRO.",
    excerpt: "For decades, SAP security and audit teams anchored configuration oversight around SPRO.",
    image: configurationWithoutSpro,
    date: "2026-01-17",
    category: "sap-public-cloud",
    author: "Raghu Boddu",
    views: "48",
    tags: ["Auditing SAP Public Cloud", "CPC", "FAQs on Public Cloud", "SAP Security in SAP Public Cloud", "SPRO"]
  },
  {
    id: "CpcVsSpro",
    title: "CPC vs. SPRO: A Security – Centric View of SAP Configuration",
    slug: "cpc-vs-spro-a-security-centric-view-of-sap-configuration",
    metaDescription: "A Security – Centric View of SAP Configuration: CPC vs. SPRO.",
    excerpt: "For a long time, SAP teams have relied on SPRO as the primary entry point for understanding configuration.",
    image: cpcVsSpro,
    date: "2026-01-17",
    category: "sap-public-cloud",
    author: "Raghu Boddu",
    views: "30",
    tags: ["Centralized Parameter Configuration", "CPC", "CPC in SAP S/4HANA Public Cloud", "SAP Public Cloud", "SPRO", "SPRO_ADMIN"]
  },
  {
    id: "PublicVsPrivateCloud",
    title: "S/4HANA Public Cloud vs. Private Cloud: A Security-Centric Perspective",
    slug: "s-4hana-public-cloud-vs-private-cloud-a-security-centric-perspective",
    metaDescription: "A Security-Centric Perspective on S/4HANA Public vs Private Cloud.",
    excerpt: "SAP’s cloud strategy is no longer aspirational—it is directive.",
    image: publicVsPrivateCloud,
    date: "2026-01-05",
    category: "sap-public-cloud",
    author: "Raghu Boddu",
    views: "256",
    tags: ["S/4HANA Public Cloud", "Private Cloud", "SAP Security", "Cloud Strategy"]
  },
  {
    id: "CybersecurityInsights",
    title: "SAP Cybersecurity Insights from the Authors of Cybersecurity for SAP book by SAP Press",
    slug: "sap-cybersecurity-insights-from-the-authors-of-cybersecurity-for-sap-book-by-sap-press",
    metaDescription: "SAP Cybersecurity Insights from the Authors of Cybersecurity for SAP book.",
    excerpt: "Sharing Episode 2 of the Cyber Kriya Podcast, featuring Juan Perez-Etchegoyen (JP) and Gaurav Singh.",
    image: cybersecurityPodcast,
    videoID: "q_YclMMJogA",
    date: "2026-01-17",
    category: "podcasts",
    author: "Raghu Boddu",
    views: "24",
    tags: ["CyberKriya", "CyberKriyaPodcast", "CybersecurityForSAP", "InfoSec", "S4HANA", "SAPAudit", "SAPGRC", "SAPSecurity"]
  },
  {
    id: "TheMagicianAndMachine",
    title: "The Magician, the Machine, and SAP Cybersecurity",
    slug: "the-magician-the-machine-and-sap-cybersecurity",
    metaDescription: "The Magician, the Machine, and SAP Cybersecurity.",
    excerpt: "In this post, we are featuring an insightful podcast episode from the CyberKriya Podcast.",
    image: magicianMachine,
    videoID: "jUgw-s7f6IQ",
    date: "2026-01-06",
    category: "podcasts",
    subCategory: "sap-cybersecurity",
    author: "Raghu Boddu",
    views: "282",
    tags: ["Agentic AI", "AI Security", "Cyber Risk Management", "CyberKriya Podcast", "Digital Trust", "Enterprise Security", "Raghu Boddu", "Repudiation Risk", "SAP AI", "SAP Community", "SAP Cybersecurity", "SAP GRC", "SAP Security", "SAPSecurityExpert", "Security Leadership", "Threat Modeling"]
  },
  {
    id: "LicensingOptimization",
    title: "SAP Licensing Optimization: Why “License Saver” Tools Often Create False Savings",
    slug: "sap-licensing-optimization-why-license-saver-tools-often-create-false-savings",
    metaDescription: "Why “License Saver” Tools Often Create False Savings in SAP Licensing.",
    excerpt: "SAP license optimization has evolved from a procurement exercise into a strategic governance concern.",
    image: licenseOptimization,
    date: "2026-01-17",
    category: "sap-licensing",
    subCategory: "other-tools",
    author: "Raghu Boddu",
    views: "140",
    tags: ["License Saver", "Licensing Audit", "Licensing Audit Bots", "Licensing Reports", "SAM4U", "SAP License Optimization", "SAP License Optimizer", "SAP License Saver", "SLAW", "STAR", "USMM"]
  },
  {
    id: "WhatActuallyOptimizes",
    title: "What Actually Optimizes SAP Licenses: STAR, USMM, LAW/SLAW Explained",
    slug: "what-actually-optimizes-sap-licenses-star-usmm-law-slaw-explained",
    metaDescription: "STAR, USMM, LAW/SLAW Explained for SAP Licensing Optimization.",
    excerpt: "SAP license compliance is not driven by a single report or transaction. It is the outcome of multiple SAP-delivered mechanisms.",
    image: starUsmmLawSlaw,
    date: "2026-01-07",
    category: "sap-licensing",
    author: "Raghu Boddu",
    views: "138",
    tags: ["LAW", "S/4HANA Licensing", "SAP Audit Preparation", "SAP Audit Readiness", "SAP Authorization Analysis", "SAP Compliance Automation", "SAP License Administration Workbench", "SAP License Compliance", "SAP License Cost Optimization", "SAP License Governance", "SAP License Intelligence", "SAP License Measurement", "SAP License Optimization", "SAP License Optimization Tools", "SAP License Risk Management", "SAP License Savings", "SAP License Strategy", "SAP Licensing Best Practices", "SAP role design", "SAP Self Assessment", "SAP STAR Analysis", "SAP Tool Rationalization", "SLAW", "USMM"]
  }
];

// Preload images with error handling
if (typeof Image !== 'undefined') {
  blogMetadata.forEach(blog => {
    const img = new Image();
    img.src = blog.image;
    // img.onload = () => console.log(`Successfully preloaded image: ${blog.image}`);
    img.onerror = () => {
      // console.error(`Failed to preload image: ${blog.image}`);
    };
  });
}