...
try {
    ...
    $stmt = $pdo->prepare("SELECT id, name, email, username, phone, location, profile_image, company_name, job_role, receive_blog_emails FROM members WHERE email = ? LIMIT 1");
    $stmt->execute([$email]);
    $member = $stmt->fetch();
